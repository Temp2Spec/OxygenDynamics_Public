...
for datai=1:length(Paths)
    ...
    switch answer1
        ...
        case 'Only df/f tifs'
            ...
            if not(isfolder('OxygenSinks_Output')) || strAgain == 'Y' || strOW == 'Y' %the last one is redundant. There is a check in the master analysis script
                ...
                run OxygenDynamics_Tiffout % call the analysis script
        ...
        case 'All analysis'
            ...
            if not(isfolder('OxygenSinks_Output')) || strAgain == 'Y' || strOW == 'Y' %the last one is redundant. There is a check in the master analysis script
                ...
                try
                    ...
                    run OxygenDynamics_Master % call the analysis script        
                    ...
                    OxyDynamicsfailures(end + 1).FileName = [Paths{datai}];           
                    OxyDynamicsfailures(end).ERROR  = getReport(ME); 
                    OxyDynamicsfailures(end).MESSAGE  = [ME.message];
    ...
    clearvars -except datai Masterfolder Paths Mice DrugIDs Conditions Genotypes Promoters SampleFs Pixelsizes Postures Pupils Puffs strOW strAgain InputD source...
        destination myEmailPassword setpref props answer1 OxyDynamicsfailures Behaviourfailures
end


% If there were any failures, save the files that contain the info
if ~isempty(OxyDynamicsfailures)

    save('OxyDynamicsfailures.mat','OxyDynamicsfailures');
end
...
msg = 'OxygenDynamics_Wrapper finished running successfully';     % main body of email.   
